  ...
  try {
    ...
    throw new FontRepositoryError(
      ...
      error instanceof Error && error.name === "AbortError"
        ? translate("errors.fonts.downloadTimeout")
        : translate("errors.fonts.downloadNetwork"),
      { cause: error },
    );
  } finally {
    clearTimeout(timer);
...
export async function downloadFontFamily(
  ...
  for (const face of family.faces) {
    ...
    if (!response.ok) {
      throw new FontRepositoryError(
        "invalid-font",
        translate("errors.fonts.downloadHttp", { status: response.status }),
      );
    }
    ...
    if (Number.isFinite(contentLength) && contentLength > face.byteLength) {
      throw new FontRepositoryError(
        "integrity-mismatch",
        translate("errors.fonts.downloadTooLarge"),
      );
    }
  ...
  if (!installed) {
    throw new FontRepositoryError(
      "invalid-font",
      translate("errors.fonts.catalogMissing"),
    );
  }
  return installed;
}
