...
应用层与渲染层只实现 iOS 和 Android；存储、授权、文件选择、Skia 资源生命周期与翻页均直接走 Native 路径。
...
新设备下载缺失书籍时，底部浮层显示真实的“已完成本数 / 总本数”和确定型进度条。`SyncEngine`
仍保持每本 EPUB 独立下载、校验和原子导入；每本完成后先应用该书的远端阅读位置，再通知应用刷新书架，因此书卡会逐本出现并立即可读，不必等待整轮同步结束。下载顺序优先采用最近有阅读进度的书，其余按加入时间排序；中途失败不会隐藏已经完整导入的书。
...
已授权设备会在冷启动、回到前台和前台每 60 秒同步；导入、删除或阅读位置变化后再以 1.5 秒合并窗口触发同步。首次关联的新设备在连接成功后立即执行同一条 sync 路径：合并远端设备文档、按优先级逐本下载缺失 EPUB、原子导入并应用对应远端进度，然后把采用后的状态写进自己的设备文档。远端状态因此不会只存在于一次运行的内存里。
...
shader，由 Native 按屏幕列逐片元求逆。自动翻页包含“压入成弧 → 荡过书脊 → 贴着落纸页滚开”；纸卷落到目标页之后书脊端切线钉在该页上，所以纸不可能穿进页面里。右侧拖拽可保持预压形态，松手按距离、速度和 page
...
是这套模型的唯一定义；UI runtime 与 Native 逐片元求逆路径都从它取值：
